...
to go ;; forever button
  if ((turtles with [guard? = TRUE]) = turtles)
    [
      ;create output file
      file-open "SAD-Model4.csv"

      file-type (word large-perc ",")
      file-type (word small-perc ",")
      file-type (word not-enough-perc ",")
      file-type (word enough-wide-perc ",")
      file-type (word enough-narrow-perc ",")
      file-type (word num-wide-guards ",")
      file-type (word num-narrow-guards ",")
      file-type (word num-ants ",")
      file-type (word large-narrow ",")
      file-type (word large-wide ",")
      file-type (word small-narrow ",")
      file-type (word small-wide ",")
      ...
      [
        ...
        [
          file-type (word ([number-of-guards] of patch-1) ",")
        ]
      ]
      file-type ("\n")
      file-close
...
to-report patch-good-enough [a-patch] ;; turtle procedure
  ...
  [report FALSE]
  ifelse [guarded?] of a-patch = FALSE
  [ifelse [large?] of a-patch = TRUE
    [ifelse (random-float 1 < large-perc)
      [report TRUE]
      [report FALSE]]
    [ifelse (random-float 1 < small-perc)
      [report TRUE]
      [report FALSE]]]
  [ifelse [narrow?] of a-patch = TRUE
    [ifelse ([number-of-guards] of a-patch) < num-narrow-guards
      [ifelse (random-float 1 < not-enough-perc)
        [report TRUE]
        [report FALSE]]
      [ifelse (random-float 1 < enough-narrow-perc)
        [report TRUE]
        [report FALSE]]]
    [ifelse ([number-of-guards] of a-patch) < num-wide-guards
      [ifelse (random-float 1 < not-enough-perc)
        [report TRUE]
        [report FALSE]]
      [ifelse (random-float 1 < enough-wide-perc)
        [report TRUE]
        [report FALSE]]]]
